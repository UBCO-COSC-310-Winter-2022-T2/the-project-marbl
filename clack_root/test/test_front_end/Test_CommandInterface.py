...
def test_register():
    ...
    def mock_func(email, password, username, first_name, last_name):
        if(email == 'valid@valid.com' and len(password) > 6): # firebase will not check validity of username, firstname, lastname
    ...
    r1 = cmd.createAccount('valid@valid.com', 'valid', 'validuser', 'validfirst', 'validlast') # success
    r2 = cmd.createAccount('invalid', 'invalid', 'valid', 'validuser', 'validfirst', 'validlast') # fail (invalid email)
    r3 = cmd.createAccount('valid@valid.com', 'valid','','','') # fail (blank fields)
    r4 = cmd.createAccount('valid@valid.com', 'valid','validuser','validfirst','') # fail (last name must be 1+ letters)
    r5 = cmd.createAccount('valid@valid.com', 'valid','validuser','','validlast') # fail (first name must be 1+ letters)
    r6 = cmd.createAccount('valid@valid.com', 'valid','a','validfirst','validlast') # fail (username must be 3+ letters)
    assert r1 and r1["success"] == True
    assert r2 and r2["success"] == False
